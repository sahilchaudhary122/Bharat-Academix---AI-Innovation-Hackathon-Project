...
// Your existing Sahil student UUID from Supabase
...
function displayStudent(student) {
    ...
    studentGoal.textContent =
        student.learning_goals || "No learning goal specified.";
...
function displayProgress(progress) {
    ...
    if (!progress || progress.length === 0) {
        ...
        progressContainer.innerHTML =
            `<div class="empty">No learning progress recorded yet.</div>`;
    ...
    progress.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="progress-details">
                ...
                <span>Strength: ${escapeHTML(strength)}</span>
            </div>

            <div class="progress-details">
                <span>Weakness: ${escapeHTML(weakness)}</span>
            </div>
        `;
...
function displayAssessments(assessments) {
    ...
    if (!assessments || assessments.length === 0) {
        ...
        assessmentContainer.innerHTML =
            `<div class="empty">No assessments completed yet.</div>`;
...
function displayLessons(lessons) {
    ...
    if (!lessons || lessons.length === 0) {
        ...
        lessonsContainer.innerHTML =
            `<div class="empty">No previous lessons yet.</div>`;
...
lessonForm.addEventListener("submit", async function(event) {
    ...
    if (!subject || !topic || !time) {
        showError("Please fill in all lesson fields.");
        return;
    }
    ...
    try {
        ...
        // Use the student's dashboard information
        // so the request matches the actual student.
        ...
        // Refresh previous lessons so the newly
        // generated lesson appears in the dashboard.
        await loadDashboard();
...
function displayGeneratedLesson(lesson) {
    ...
    lessonTitle.textContent =
        lesson.title || "Personalized Lesson";
    ...
    generatedLesson.scrollIntoView({
        behavior: "smooth",
        block: "start"
    });
}
